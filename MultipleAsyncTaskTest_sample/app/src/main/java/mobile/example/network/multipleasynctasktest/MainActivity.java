...
//        Input data : 주소창에 입력한 주소(String)
       @Override
       protected String doInBackground(String... strings) {
           String url = strings[0]; //주소 창에 입력한 주소 (url)
...
//         수신한 문자열 반환
       ...
       protected void onPostExecute(String s) {
           tvImgLink.setText(s);
           btnImgDownload.setEnabled(true); //버튼 활성화
       }
   }
   ...
   /* 문자열 형태의 주소를 전달 받아 네트워크에서 비트맵 데이터(이미지)를 받아옴
    ...
    class ImageAsyncTask extends AsyncTask<String, Integer, Bitmap> {
        ...
        protected Bitmap doInBackground(String... strings) {
            String imgLink = strings[0]; //이미지 주소
            Bitmap image = downloadImage(imgLink);
            return image;
        }
    ...
    class DirectDownAsyncTask extends AsyncTask<String, Integer, String> {
        @Override
        protected String doInBackground(String... strings) {
            String url = strings[0]; //url
            String imgLink = downloadContents(url); //이미지 주소
//            내부에서 UI 요소 접근 불가하므로 다른 asynctask를 이용하여 이미지를 띄움
...
//            inputstream을 string으로 변환
    ...
    protected String readStreamToString(InputStream stream){
        ...
        try {
//            inputstream 데이터를 inputstreamreader로 읽어서
            InputStreamReader inputStreamReader = new InputStreamReader(stream);
//            bufferedreader가 대용량 inputstream을 효율적으로 읽어옴
