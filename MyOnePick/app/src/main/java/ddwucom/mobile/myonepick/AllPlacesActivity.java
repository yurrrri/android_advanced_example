...
public class AllPlacesActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        lvOnePicks = (ListView) findViewById(R.id.lvOnePicks);

        helper = new PlaceDBHelper(this);

        adapter = new MyCursorAdapter(this, R.layout.listview_layout, null);
        lvOnePicks.setAdapter(adapter);

//		리스트 뷰 클릭 처리
        lvOnePicks.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                Intent intent = new Intent(AllPlacesActivity.this, UpdatePlaceActivity.class);
...
//		리스트 뷰 롱클릭 처리
        lvOnePicks.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
            @Override
            public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
                final long _id = id;
                AlertDialog.Builder builder = new AlertDialog.Builder(AllPlacesActivity.this);
                builder.setTitle("원픽 삭제")
                        .setMessage("삭제하시겠습니까?")
